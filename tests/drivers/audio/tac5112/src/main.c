...
ZTEST_F(tac5112, test_set_property_rejects_unsupported_channel_and_property)
{
	...
	zassert_equal(audio_codec_set_property(fixture->dev, AUDIO_PROPERTY_OUTPUT_VOLUME,
		      ...
		      -ENOTSUP);
	zassert_equal(
		audio_codec_set_property(fixture->dev, AUDIO_PROPERTY_EQ_GAIN,
					  AUDIO_CHANNEL_ALL, val),
		-ENOTSUP);
}
...
ZTEST_F(tac5112, test_set_property_rejects_null_dev)
{
	...
	zassert_equal(api->set_property(NULL, AUDIO_PROPERTY_OUTPUT_VOLUME,
					 AUDIO_CHANNEL_ALL, val),
		      -EINVAL);
}
...
ZTEST_F(tac5112, test_configure_enables_fault_interrupts)
{
	...
	zassert_equal(val & (TAC5112_INT_MASK4_OUT_SC_BIT | TAC5112_INT_MASK4_DRVR_VG_BIT),
		      0, "OUT SC / virtual-ground fault interrupts must be unmasked");
...
ZTEST(tac5112, test_dvol_conversion_dac_range)
{
	...
	zassert_equal(
		tac5112_dvol_from_half_db(true, TAC5112_DAC_VOL_MIN_HALF_DB - 1, &dvol),
		-EINVAL);
	zassert_equal(
		tac5112_dvol_from_half_db(true, TAC5112_DAC_VOL_MAX_HALF_DB + 1, &dvol),
		-EINVAL);
}
...
ZTEST(tac5112, test_dvol_conversion_adc_range)
{
	...
	zassert_equal(
		tac5112_dvol_from_half_db(false, TAC5112_ADC_VOL_MAX_HALF_DB + 1, &dvol),
		-EINVAL);
}
