...
int tac5112_configure_pll(const struct device *dev, const struct tac5112_pll_config *pll)
{
...

	{
		...
		uint8_t clk_cfg16 = (uint8_t)((jmul_msb != 0U ? TAC5112_PLL_JMUL_MSB_BIT : 0U) |
					       dmul_msb);
	...
	ret = tac5112_reg_write(dev, TAC5112_REG_CLK_CFG20,
				 (uint8_t)((pll->mdiv << TAC5112_MDIV_SHIFT) |
					   pll->adc_modclk_div_sel));
...
static int tac5112_configure_dai(const struct device *dev, const struct audio_codec_cfg *cfg)
{
	...
	val = (uint8_t)((format << TAC5112_PASI_FORMAT_SHIFT) |
			 (wlen << TAC5112_PASI_WLEN_SHIFT));
...
static int codec_configure(const struct device *dev, struct audio_codec_cfg *cfg)
{
	 ...
	 */
	ret = tac5112_reg_write(dev, TAC5112_REG_PWR_CFG,
				 (uint8_t)(TAC5112_PWR_CFG_ADC_PDZ_BIT |
					   TAC5112_PWR_CFG_MICBIAS_PDZ_BIT));
	if (ret < 0) {
		goto unlock;
	}
	...
	ret = tac5112_reg_update(dev, TAC5112_REG_INT_MASK4,
				  (uint8_t)(TAC5112_INT_MASK4_OUT_SC_BIT |
					    TAC5112_INT_MASK4_DRVR_VG_BIT),
				  0U);
	if (ret < 0) {
		goto unlock;
	}
 ...
 */
static int tac5112_apply_property(const struct device *dev, audio_property_t property,
				   uint8_t idx, audio_property_value_t val)
{
	...
	if (is_output) {
		struct tac5112_reg reg_a = (idx == 0U) ? TAC5112_REG_DAC_CH1A_CFG0
							 : TAC5112_REG_DAC_CH2A_CFG0;
		struct tac5112_reg reg_b = (idx == 0U) ? TAC5112_REG_DAC_CH1B_CFG0
							 : TAC5112_REG_DAC_CH2B_CFG0;
	...
	{
		struct tac5112_reg adc_reg = (idx == 0U) ? TAC5112_REG_ADC_CH1_CFG2
							   : TAC5112_REG_ADC_CH2_CFG2;
			       ...
			       audio_channel_t channel, audio_property_value_t val)
{
	...
	if ((property != AUDIO_PROPERTY_OUTPUT_VOLUME) &&
	    (property != AUDIO_PROPERTY_OUTPUT_MUTE) &&
	    (property != AUDIO_PROPERTY_INPUT_VOLUME) &&
...
static void tac5112_fault_isr(const struct device *port, struct gpio_callback *cb,
			       uint32_t pins)
{
	struct tac5112_data *data;
...
static int codec_register_error_callback(const struct device *dev,
					  audio_codec_error_callback_t cb)
{
	struct tac5112_data *data;
	const struct tac5112_config *cfg;
...
static int tac5112_init(const struct device *dev)
{
	...
	if (gpio_is_ready_dt(&cfg->fault_gpio)) {
		...
		ret = gpio_pin_interrupt_configure_dt(&cfg->fault_gpio,
...
/*
 * This Zephyr version exposes the codec API as a plain
 * "struct audio_codec_api" and does not provide the DEVICE_API() wrapper
 * / the "audio_codec_driver_api" tag that later mainline revisions use.
 */
static const struct audio_codec_api tac5112_driver_api = {
