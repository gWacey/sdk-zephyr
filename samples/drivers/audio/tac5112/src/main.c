...
static int codec_setup(void)
{
	...
	LOG_INF("codec configured: I2S target, %u Hz, %u-bit, %u ch",
		AUDIO_SAMPLE_RATE_HZ, AUDIO_WORD_BITS, AUDIO_CHANNELS);
	return 0;
}
...
static void tone_block_init(void)
{
	for (uint32_t i = 0; i < SAMPLES_PER_BLOCK; i++) {
		...
		int16_t s = (int16_t)(0.25 * 32767.0 *
				      sin(2.0 * 3.14159265358979 * TONE_FREQ_HZ * t));
...
static int run_loopback(void)
{
	...
	LOG_INF("ADC->DAC loopback for %u s (varying output volume + mute)",
		LOOPBACK_SECONDS);
